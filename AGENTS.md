...
- **Trunk = `feature/lovable-migration`.** It receives changes ONLY via pull requests — guaranteed by the server-side `trunk-pr-only` ruleset; the local guard hook is best-effort fast feedback on top (its known gaps are documented in the hook header and all land on the server-side wall). PR #6 to `main` stays open until full cutover.
