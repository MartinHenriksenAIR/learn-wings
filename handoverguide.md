...
### The `/cavemem` command — session state persistence

The `/cavemem` slash command provides an additional durable memory layer specifically for migration progress and infrastructure facts. When invoked, it writes the current task completion state to `~/.claude/projects/.../memory/project_migration_status.md`, a persistent file that survives across sessions and is available as project memory in future Claude Code sessions.

**When to use it:** After any task completes, or when the session context is getting long. Invoking it takes seconds and can save re-discovery time at the start of the next session.

**How to invoke it:** Type `/cavemem` at the Claude Code prompt.

**The TaskUpdate hook:** A `PostToolUse` hook in `.claude/settings.json` fires whenever a task is marked `completed` via the `TaskUpdate` tool. It emits a `systemMessage` nudge: *"Task marked complete — run /cavemem to persist migration state to memory."* The hook does not auto-save — it reminds. The user or agent still needs to invoke `/cavemem` manually.

**The hard-block limitation:** Writing to `.claude/commands/` is blocked by Claude Code's auto-mode classifier, which treats writes to that directory as agent self-modification. The agent can provide the file content but cannot create the file. The user must create `.claude/commands/cavemem.md` manually via terminal. This is the same class of constraint as the `.mcp.json` hard-block (see Section 9.5 and Failure 5) — some infrastructure setup files require human action.

*Note: `/cavemem` and the TaskUpdate hook were set up at the start of 2026-05-20. They were not exercised during the main session window documented in this guide. The mechanism is available but the persistent memory file may not yet contain migration state entries.*
...
Durable (survives sessions):
  ...
  project_migration_status → Written by /cavemem; task completion + infra facts
...
| `.claude/commands/cavemem.md` | Defines the `/cavemem` slash command for persisting migration state to project memory | Created manually by user (agent hard-blocked from writing to `.claude/commands/`) |
| `.claude/settings.json` | Project-level Claude Code settings; contains the `PostToolUse` hook on `TaskUpdate` that nudges `/cavemem` on task completion | Modified by agent (hook was added 2026-05-20) |
