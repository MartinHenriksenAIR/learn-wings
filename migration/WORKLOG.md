...
## 2026-08-13 — #451 orphan sweep has never completed a run (PR #453)

**Who:** claude (Opus 5, 1M) with emil. Not from the board — emil brought the nightly "Orphan sweep refused to run" emails, on night 17 of what turned out to be a 19-night streak. Branched off `origin/main` @`751bbb7`.

**What:** the nightly blob sweep aborted on **every run it ever made** — first scheduled occurrence 2026-07-26 03:00 UTC (shipped 2026-07-25, PR #279), 19 consecutive `orphan-bucket-share-implausible` aborts, **0 blobs ever reclaimed**. The alerting (#286) worked correctly and mailed all 19 nights; what nobody could do was act on them.

**Root cause — a self-perpetuating tripwire, not a reconciliation bug.** `avatars/` at 3/5 unreferenced (60%) and `org-logos/` at 3/4 (75%) sat above the 50% per-bucket ceiling. The six blobs are **genuine orphans** — confirmed against every `text`/`varchar` column in `public`, not just the six in `REFERENCED_PATHS_SQL` — and the reconciliation is exact, since 3 of the 9 branding blobs matched verbatim (a broken match would have made all 9 look orphaned). They are ordinary replaced/cancelled uploads; three are byte-identical (1,788,462 B) to a live avatar, i.e. the same file uploaded three times with two attempts stranded. Nothing but the sweep deletes them, and they are what made the sweep refuse — **the condition could never clear itself.**

This is precisely the deadlock `MIN_ROOT_CLASS_ORPHANS` was added to prevent one level down in #282, whose own docblock says it verbatim: *"keeps aborting it every night forever, because the only thing that could clear the blob is the sweep the blob is blocking."* The floor was applied to the root file-type classes and deliberately withheld from the prefix buckets, on two premises production falsified: that a nuisance abort there costs *"one night"* (it costs every night, permanently), and that those are namespaces a writer *"keeps populated"* so a whole-namespace orphan share is anomalous at any size. Branding namespaces hold at most **one live asset per user and per org**; every replacement strands its predecessor, so their orphan share climbs toward 100% by construction and two ordinary logo changes clear a 50% ceiling on a 4-blob bucket. Underneath both: the TRIPWIRE docblock describes designing against *"a few thousand root-level blobs against a few hundred `avatars/` + `org-logos/`"*. The container holds **26 blobs** — percentage tests calibrated for a container two orders of magnitude larger than the one they guard.

**Fix:** (1) `MIN_ROOT_CLASS_ORPHANS` → `MIN_BUCKET_ORPHANS` (unchanged at 5), now applied to every **prefix** bucket as well as the root classes. The undivided container root stays **unfloored** — after #282's split it is the last fully-sensitive check root-level blobs have, and it is the aggregate of a namespace live writers keep genuinely populated. What the extension gives up is stated in the docblock rather than left implicit: a whole-column break inside a prefix bucket that never held more than four blobs, bounded to those four and backstopped by 7-day soft-delete plus the deletion digest. (2) `OrphanSweepSummary.abortDetail` carries the full refusal, composed once so the log line and the email cannot drift, persisted on `orphan_sweep_runs.abort_detail` (migration `16-orphan-sweep-abort-detail.sql`) and rendered in the alert email.

**Why (2) matters more than (1):** the detail previously existed only inside a `log.error`, so acting on any refusal depended on log ingestion working — and it silently was not. The App Insights component `func-ai-education-migration` is workspace-based against `DefaultWorkspace-…-SEC` in `DefaultResourceGroup-SEC`; `az group exists` on that RG returns **false**, the component reports `IngestionMode: Disabled`, and `traces`/`requests` return zero rows over 21 days. Every one of the 19 emails told its reader to run a query that returns nothing. **This is fleet-wide, not sweep-specific — the whole function app has no working telemetry** — and is filed as **#452**; it needs an Azure change, so no code here touches it. (1) unwedges this instance; (2) is what makes the next wedge actionable on night 1 without App Insights.

**Diagnosis method worth reusing:** the census was reproduced **read-only** against prod storage + Postgres using the *deployed build's own* `parseListPage` / `referenceVariants` / `blobBuckets`, so the classification was identical to what runs at 03:00 rather than a re-implementation that could disagree. That is what turned "which bucket?" — a question the emails could not answer — into a table in minutes. The full-schema UUID search (every text column in `public`) is what upgraded "the six columns don't reference these" into "nothing anywhere does", which is the claim that actually licenses deleting them.

**Verify:** root `lint` 0 errors · `tsc` app+node 0 · `npm test` **948 / 125** · `build` 0; functions `build` 0 · `npm test` **2756 / 158** (3 skip). Eight new tests, including `sweeps the exact production container that wedged the job for 19 nights` (the real 26-blob census reproduced blob for blob) and `still aborts on a prefix bucket once the break is big enough to be evidence` (5/5 avatars still trips, so the floor is not a blanket exemption). The test that asserted the *old* decision — `still aborts on a TWO-blob prefix bucket — the root-class floor is not applied here` — was rewritten rather than deleted, so the reversal is visible in the diff. Re-running the census with the new rule against **live prod** leaves no bucket over the ceiling; the first run after deploy should reclaim 12 blobs.

**Deploy:** migration `16-*.sql` must be applied to prod **before** merge (`notify.ts` INSERTs the column on every run). Backend-only otherwise.

**Watch on the next run:** the GLOBAL share is **46.2%** against its own 50% ceiling. Two more stranded uploads before this deploys and the run wedges again on `orphan-share-implausible` — different reason, same permanence. One successful run collapses the census to near-zero orphans and restores the margin, so getting a first run through is the whole priority. If it does trip, the fix is *not* to raise `ORPHAN_SWEEP_MAX_SHARE` (the abort's own remedy text warns against that, correctly) — it is that the global share has the same population-size problem the buckets had, and a 26-blob container cannot support a percentage test.
