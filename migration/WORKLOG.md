...
- **dead-4/#259:** stale `bun.lock` + `bun.lockb` deleted (months behind package.json; npm-only toolchain) — also removes Oryx package-manager-inference ambiguity. Watch the PR preview build stays npm (it did).
...
**Deploy:** merging auto-fires both workflows. Deploy + signed-in /ui-report smoke over the touched surfaces (community pages, org-admin idea/moderation gates) announced on PR #260. Board HTML on Desktop updated (32 open cards remain).
