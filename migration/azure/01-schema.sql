...
-- ---- course_assignments (issue #365) ----
-- An admin assigns a course to a learner (user_id set) or a whole org
-- (user_id NULL — applies to current AND future active members, resolved at
-- read time). mandatory=false means "recommended". due_date is optional and
-- informational (drives an overdue badge; nothing is blocked after it passes).
...
-- One individual assignment per (org, user, course); one whole-org per (org, course).
CREATE UNIQUE INDEX course_assignments_individual_uniq
  ON public.course_assignments (org_id, user_id, course_id) WHERE user_id IS NOT NULL;
CREATE UNIQUE INDEX course_assignments_org_uniq
  ON public.course_assignments (org_id, course_id) WHERE user_id IS NULL;
CREATE INDEX course_assignments_org_idx    ON public.course_assignments (org_id);
CREATE INDEX course_assignments_user_idx   ON public.course_assignments (user_id) WHERE user_id IS NOT NULL;
CREATE INDEX course_assignments_course_idx ON public.course_assignments (course_id);
...
-- Seed: the hidden "Individuals" self-serve placeholder org (#354). Fixed id for
-- idempotent seeding; detection in code is by kind='individual', never this id.
-- name doubles as the certificate issuer for solo learners. Unlimited members.
