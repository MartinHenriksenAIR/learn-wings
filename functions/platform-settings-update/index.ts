...
const ALLOWED_KEYS = ['branding', 'user_access', 'features', 'seat_pricing'] as const;
...
export default adminEndpoint('platform-settings-update', async ({ req, profile, reply }) => {
  ...
  if (typeof body.key !== 'string' || !(ALLOWED_KEYS as readonly string[]).includes(body.key)) {
    return reply(400, { error: 'key must be one of: branding, user_access, features, seat_pricing' });
  }
  ...
  // the stored branding colors). updated_at is managed by a DB trigger on
