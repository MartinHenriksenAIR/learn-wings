...
describe('azure-delete-blob', () => {
  ...
  it('returns a generic 500 (no leak) when a non-auth error mentions "token"', async () => {
    mockDeleteBlob.mockRejectedValueOnce(new Error('storage token expired'));
    ...
    expect(ctx.error).toHaveBeenCalledWith(expect.stringContaining('storage token expired'));
  });
