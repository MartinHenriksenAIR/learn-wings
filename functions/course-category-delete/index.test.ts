...
const validBody = { categoryId: 'cat-1' };

describe('course-category-delete', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('returns 400 when categoryId is not a string', async () => {
    const res = await handler(baseReq({ categoryId: 99 }), {} as any);
    expect(res.status).toBe(400);
    expect(JSON.parse(res.body as string)).toEqual({ error: 'categoryId is required' });
  });

  it('happy path: deletes the row and returns success', async () => {
    ...
    expect(sql).toContain('DELETE FROM course_categories');
