...
import { useLearnerTraining } from './useLearnerTraining';
...
function Consumer({ orgId, enabled }: { orgId: string | undefined; enabled?: boolean }) {
  const query = useLearnerTraining(orgId, enabled !== undefined ? { enabled } : {});
...
describe('useLearnerTraining', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('calls /api/learner-training with the correct { orgId } body', async () => {
    ...
    await waitFor(() => {
      expect(mockCallApi).toHaveBeenCalledWith('/api/learner-training', { orgId: 'org-1' });
    });
  });
