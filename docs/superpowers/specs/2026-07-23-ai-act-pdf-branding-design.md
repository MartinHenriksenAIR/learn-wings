...
5. **Content → concise, conclusion-first, Article-4 framing.** Ruthlessly lean — the verdict is legible in the first third of one page; no vanity metrics. Scoped honestly as *training* evidence, **not** a full AI-Act conformity assessment. **Headline metric = share of active staff who have completed ≥1 AI-literacy course** ("training participation") — deliberately well-defined, so it needs **no "required course" concept and no schema change**; the assessed-literacy distribution (§4) is the depth companion.
...
5. **§1 Summary** — key-figures strip (Staff in scope · Staff trained · **Participation %** · Not yet trained · Refresher due) + a one-line status sentence with the status word (e.g. "Action required") in oxblood small-caps.
6. **§2 Coverage by department** — ruled table: Department · Staff · Trained (≥1 course) · Participation · Assessed level · Status (On track / Below target / Priority; deficiencies in oxblood). Stated ≥80% target note.
7. **§3 Course completion** — compact ruled table: each AI-literacy course · completion %.
...
The richer content needs more than today's dept/course aggregation, but every figure maps to existing data via additive queries — **no schema change**:
- **Training participation (headline) / trained / not-yet-trained** — distinct active staff with ≥1 `enrollments.status = 'completed'`, over total active staff; joined to `profiles.department` for the per-dept table. Well-defined; no "required course" concept.
- **Course completion (§3)** — per-course completed/enrolled over the org's AI-literacy courses.
- **Assessed literacy (§4)** — `profiles.assessment_level` + `assessment_skipped_at` (#117): counts per level incl. "Not assessed".
- **Refresher due** — staff whose most recent completion timestamp on `enrollments` is older than 12 months.
...
- **Files touched:** `functions/generate-compliance-report/*` (+ `strings.ts`, `logo.ts`, test) and one line in `src/pages/org-admin/OrgAnalytics.tsx`. No schema change, no admin UI. None overlap #226 (`i18n/index.ts`, `user-context`, `useAuth`, `Signup`, `Settings`) or #227 (`CourseEditor`, `CoursePlayer`, `types.ts`, locale JSON, schema `lesson_type`, `validate.ts`, lesson endpoints).
