...
- **Tests:** mock contract tests per endpoint (`*/index.test.ts`): happy path + 401/403 authz + key errors. Mock `shared/auth`, `shared/db`, `shared/profile`; NEVER touch a real DB. Verify: `npm --prefix functions run verify` (exit 0).
